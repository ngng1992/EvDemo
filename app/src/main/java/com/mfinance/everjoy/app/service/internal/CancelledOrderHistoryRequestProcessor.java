...
public class CancelledOrderHistoryRequestProcessor implements MessageProcessor{
	private final String TAG = this.getClass().getSimpleName();
	@Override
	public boolean processMessage(Message msg, FxMobileTraderService service) {
		Bundle data = msg.getData();
		
		String sFrom = data.getString(ServiceFunction.SRV_SEND_CANCELLED_ORDER_HISTORY_REQUEST_FROM);
		String sTo = data.getString(ServiceFunction.SRV_SEND_CANCELLED_ORDER_HISTORY_REQUEST_TO);
		BalanceRecord account = service.app.data.getBalanceRecord();
		
        try {			
			MessageObj executedOrderHistoryRequestMsg = MessageObj.getMessageObj(IDDictionary.SERVER_IO_SERVICE_TYPE, IDDictionary.SERVER_IO_REQUEST_CANCEL_ORDER_HISTORY);
		    ...
		    //Log.i(TAG, "[Sent cancelled order history request.]");
